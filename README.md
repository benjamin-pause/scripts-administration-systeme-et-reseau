...
DossierContenugeneral/Scripts génériques d'administration Linux (mises à jour, sauvegardes, utilisateurs, monitoring)reseau/Diagnostic et correction réseau (routage, Netplan, pré-checks de migration)wazuh/Vérification post-migration Wazuh, gestion des certificats Fleet Server, enrollment d'agentszabbix/Mise à jour de version, création d'hôtes en masse via l'APIelastic-docker/Healthcheck et déploiement d'une stack Elastic sous Docker Compose
...
bashchmod +x general/*.sh reseau/*.sh wazuh/*.sh zabbix/*.sh elastic-docker/*.sh
...
Benjamin PAUSE — BTS SIO option SISR, Lycée Pierre Poivre (Saint-Josehp)
